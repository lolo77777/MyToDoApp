...
public class LoginService : ILoginService
{
    ...
    public async Task<ApiResult<UserDto>> LoginAsync(string Account, string Password)
    {
        try
        {
            ...
            if (model == null)
                return Result.Fail<UserDto>("账号或密码错误,请重试！").ToApiResult();
            ...
            return Result.Ok(userDto).ToApiResult();
        }
        catch (Exception ex)
        {
            return Result.Fail<UserDto>("登录失败！").WithError(ex.Message).ToApiResult();
        }
    }

    public async Task<ApiResult> Resgiter(UserDto user)
    {
        try
        {
            ...
            if (userModel != null)
                return Result.Fail($"当前账号:{model.Account}已存在,请重新注册！").ToApiResult();
        ...
        {
            return Result.Fail("注册账号失败！").WithError(ex.Message).ToApiResult();
        }
    }
}